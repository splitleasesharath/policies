...
export const useAuthFlow = () => {
      ...
      const response = await fetch(
        'https://app.split.lease/version-test/api/1.1/wf/CORE-signup-new-user-REACT',
        {
          ...
          body: JSON.stringify({
            ...
            usertype: data.accountType,  // 'guest' or 'host' in lowercase
   ...
   * Calls Bubble backend workflow: CORE-reset-password-REACT
   */
  const resetPassword = useCallback(async (
    ...
    try {
      const response = await fetch(
        'https://app.split.lease/version-test/api/1.1/wf/CORE-reset-password-REACT',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
